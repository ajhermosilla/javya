...
- [x] ZIP archive support — Import entire song libraries at once (max 1000 files, 200MB)
- [x] OnSong format — Support for popular iOS app format (.onsong)
- [x] Smarter parsing — Auto-detect key from chord progressions, section normalization
...
## Current: v0.8 — Quality & Polish

Focus on stability, testing, and user experience improvements before adding major features.
...
### Testing & Quality
- [ ] **Frontend component tests** — Add Vitest tests for critical components
- [ ] **Export tests** — Add tests for FreeShow and Quelea exporters
- [ ] **E2E test scenarios** — Add Playwright tests for critical flows
- [ ] **Accessibility audit** — WCAG 2.1 AA compliance review

### Performance & Security
- [ ] **Rate limiting** — Protect import/upload endpoints from abuse
- [ ] **Request logging** — Add structured logging middleware
- [ ] **Database connection pooling** — Configure async pool settings
...
- **Soft deletes** — Archive instead of permanent delete for audit trail
- **Audit logging** — Track sensitive operations (role changes, deletions)
- **Bulk operations** — Select and manage multiple songs/setlists at once

---

## Technical Debt

Items to address opportunistically:

| Area | Issue | Priority |
|------|-------|----------|
| Frontend | No component tests (only hook tests) | High |
| Backend | No export tests for FreeShow/Quelea | Medium |
| Security | No rate limiting on file uploads | Medium |
| UX | No loading skeletons during fetches | Low |
| i18n | No pluralization rules | Low |
| i18n | Date/time not localized | Low |
| API | No PATCH for partial updates | Low |
| Docs | API lacks example values in OpenAPI | Low |
